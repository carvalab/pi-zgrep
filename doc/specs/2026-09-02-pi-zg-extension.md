...
**`zg` tool** — parameters (typebox): `query` (string, required); `mode`: `hybrid` (default) | `fts` | `vector` | `rg`; `limit` (number, default 10, max 50); `glob`, `type` (optional string filters); `refresh`: `auto` (default) | `wait` (forces `--refresh wait` when the agent needs post-edit freshness); `preview`: `short` (default) | `none`. Mapping: hybrid → positional query; fts → `--fts`; vector → `--vector`; rg → `--rg` (indexless); `--limit`, `--preview`, `-g`, `-t`. Two orthogonal axes, never conflated: `--mode auto` is **client transport routing** (use the server daemon when ready, else direct) and is passed on every call; the tool's `mode` param is the **search route** (`--fts`/`--vector`/`--rg`/positional). The arg builder is fixture-checked at implement start against captured `zg query --help` / `zg status --help` output (compat tripwire for the input surface, mirroring the output-parser tripwire). Execute = ensure binary → ensure index (skipped for `rg`) → spawn → parse. Details carry parsed JSON (file, line range, score, snippet) for future TUI use; content is the rendered text, one line per result in the form `path:line-start-line-end  score  first-snippet-line` (ripgrep-style), grouped under a one-line freshness note.
